  ...
  if (n1.startsWith("-") !== n2.startsWith("-")) {
    ...
    for (let ii = 1; ii >= 0; ii--) {
      for (let i = 0; i < temp1[ii].length; i++) {
        const t = String(
          Number(temp1[ii][i]) + Number(temp2[ii][i]) + carry,
        )
...
function multiplyNums(num1: string, num2: string): string {
  ...
  if (totalDec > 0) {
    ...
    str =
      str.slice(0, str.length - totalDec) +
      "." +
      str.slice(str.length - totalDec)
  }
...
function decStrToHex(
  ...
  const hexDigits =
    caseStyle === "upper" ? h.toUpperCase() : h.toLowerCase()
...
class ExprParser {
  ...
  private parseUnary(): string {
    ...
    if (
      t &&
      t.type === "op" &&
      (t as { type: "op"; value: string }).value === "-"
    ) {
      this.consume()
  ...
  private parsePrimary(): string {
    ...
    if (t.type === "num") {
      this.consume()
      return (t as { type: "num"; value: string }).value
    }
    if (t.type === "lparen") {
      this.consume()
      const val = this.parseExpr()
...
export function activate(context: vscode.ExtensionContext): void {
  const cmd = vscode.commands.registerCommand(
    "mathOnSelections.run",
    async () => {
      const editor = vscode.window.activeTextEditor
      if (!editor) {
        vscode.window.showErrorMessage(
          "Math on Selections: No active editor.",
        )
        return
      }
      ...
      if (
        selections.length === 0 ||
        selections.every((s) => s.isEmpty)
      ) {
        vscode.window.showErrorMessage(
          "Math on Selections: Make a selection first.",
        )
        return
      }
      ...
      if (!success) {
        vscode.window.showErrorMessage(
          "Math on Selections: Edit failed (document may have changed).",
        )
        return
      }
      ...
      for (const rep of replacements) {
        ...
        cumulativeDelta +=
          rep.newText.length -
          (rep.originalEndOffset - rep.originalStartOffset)
      }
